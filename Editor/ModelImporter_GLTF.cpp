...
namespace tinygltf
{
	...
	bool FileExists(const std::string& abs_filename, void*) {
		return ap::helper::FileExists(abs_filename);
	}
	...
	bool ReadWholeFile(std::vector<unsigned char>* out, std::string* err,
		...
		return ap::helper::FileRead(filepath, *out);
	}

	bool WriteWholeFile(std::string* err, const std::string& filepath,
		const std::vector<unsigned char>& contents, void*) {
		return ap::helper::FileWrite(filepath, contents.data(), contents.size());
	}
		...
		const unsigned char *bytes, int size, void *userdata)
	{
		...
		if (image->uri.empty())
		{
			...
			do {
				ss.clear();
				ss += "gltfimport_" + std::to_string(ap::random::GetRandom(std::numeric_limits<int>::max())) + ".png";
			} while (ap::resourcemanager::Contains(ss)); // this is to avoid overwriting an existing imported image
			image->uri = ss;
		}

		auto resource = ap::resourcemanager::Load(
			image->uri,
			ap::resourcemanager::Flags::IMPORT_RETAIN_FILEDATA,
			(const uint8_t*)bytes,
			(size_t)size
		);
		...
		ap::resourcemanager::ResourceSerializer* seri = (ap::resourcemanager::ResourceSerializer*)userdata;
...
struct LoaderState
{
	...
	Scene* scene;
	ap::unordered_map<int, Entity> entityMap;  // node -> entity
};
...
void LoadNode(int nodeIndex, Entity parent, LoaderState& state)
{
	...
	else if (node.camera >= 0)
	{
		...
		entity = scene.Entity_CreateCamera(node.name, ap::scene::GetCamera().width, ap::scene::GetCamera().height, 0.1f, 800);
	}
...
{
	std::string directory = ap::helper::GetDirectoryFromPath(fileName);
	std::string name = ap::helper::GetFileNameFromPath(fileName);
	std::string extension = ap::helper::toUpper(ap::helper::GetExtensionFromFileName(name));
	...
	ap::resourcemanager::ResourceSerializer seri; // keep this alive to not delete loaded images while importing gltf
	...
	ret = ap::helper::FileRead(fileName, filedata);
	...
	if (!ret) {
		ap::helper::messageBox(err, "GLTF error!");
	}
	...
	for (auto& x : state.gltfModel.materials)
	{
		...
		if (baseColorTexture != x.values.end())
		{
			...
			material.textures[MaterialComponent::BASECOLORMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (normalTexture != x.additionalValues.end())
		{
			...
			auto& img = state.gltfModel.images[img_source];
			material.textures[MaterialComponent::NORMALMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (metallicRoughnessTexture != x.values.end())
		{
			...
			auto& img = state.gltfModel.images[img_source];
			material.textures[MaterialComponent::SURFACEMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (emissiveTexture != x.additionalValues.end())
		{
			...
			auto& img = state.gltfModel.images[img_source];
			material.textures[MaterialComponent::EMISSIVEMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (occlusionTexture != x.additionalValues.end())
		{
			...
			auto& img = state.gltfModel.images[img_source];
			material.textures[MaterialComponent::OCCLUSIONMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (alphaMode != x.additionalValues.end())
		{
			...
			{
				material.userBlendMode = ap::enums::BLENDMODE_ALPHA;
			}
		}
		...
		if (ext_transmission != x.extensions.end())
		{
			...
			if (ext_transmission->second.Has("transmissionTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::TRANSMISSIONMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (ext_sheen != x.extensions.end())
		{
			...
			if (ext_sheen->second.Has("sheenColorTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::SHEENCOLORMAP].resource = ap::resourcemanager::Load(img.uri);
			...
			if (ext_sheen->second.Has("sheenRoughnessTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::SHEENROUGHNESSMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (ext_clearcoat != x.extensions.end())
		{
			...
			if (ext_clearcoat->second.Has("clearcoatTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::CLEARCOATMAP].resource = ap::resourcemanager::Load(img.uri);
			...
			if (ext_clearcoat->second.Has("clearcoatRoughnessTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::CLEARCOATROUGHNESSMAP].resource = ap::resourcemanager::Load(img.uri);
			...
			if (ext_clearcoat->second.Has("clearcoatNormalTexture"))
			{
				...
				auto& img = state.gltfModel.images[img_source];
				material.textures[MaterialComponent::CLEARCOATNORMALMAP].resource = ap::resourcemanager::Load(img.uri);
		...
		if (ext_specular != x.extensions.end())
		{
			...
			if (ext_specular->second.Has("specularTexture"))
			{
				...
				else if (!material.textures[MaterialComponent::SPECULARMAP].resource.IsValid())
				{
					...
					auto& img = state.gltfModel.images[img_source];
					material.textures[MaterialComponent::SPECULARMAP].resource = ap::resourcemanager::Load(img.uri);
				...
				{
					ap::backlog::post("[KHR_materials_specular warning] specularTexture must be either in surfaceMap.a or specularColorTexture.a! specularTexture discarded!", ap::backlog::LogLevel::Warning);
				}
			}
			...
			for (auto& attr : prim.attributes)
			{
				...
				else if (!attr_name.compare("COLOR_0"))
				{
					...
					for (size_t i = 0; i < vertexCount; ++i)
					{
						...
						uint32_t rgba = ap::math::CompressColor(color);
