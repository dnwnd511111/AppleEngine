...
public:
		...
		std::map<std::string, int>* matMap, std::string* err)
	{
		...
		if (!ap::helper::FileRead(filepath, filedata))
		{
			std::string ss;
...
void ImportModel_OBJ(const std::string& fileName, Scene& scene)
{
	...
	if (!obj_errors.empty())
	{
		ap::backlog::post(obj_errors, ap::backlog::LogLevel::Error);
	}
	...
	if (success)
	{
		...
		ap::vector<Entity> materialLibrary = {};
		...
		{
			std::string materialName = "OBJImport_defaultMaterial" + std::to_string(num++);
			// Create default material if nothing was found:
			Entity materialEntity = scene.Entity_CreateMaterial(materialName);
			MaterialComponent& material = *scene.materials.GetComponent(materialEntity);
			materialLibrary.push_back(materialEntity); // for subset-indexing...
		}
		...
		for (auto& shape : obj_shapes)
		{
			...
			ap::unordered_map<int, int> registered_materialIndices = {};
			ap::unordered_map<size_t, uint32_t> uniqueVertices = {};
			...
			for (size_t i = 0; i < shape.mesh.indices.size(); i += 3)
			{
				...
				for (auto& index : reordered_indices)
				{
					...
					size_t vertexHash = 0;
					ap::helper::hash_combine(vertexHash, index.vertex_index);
					ap::helper::hash_combine(vertexHash, index.normal_index);
					ap::helper::hash_combine(vertexHash, index.texcoord_index);
					ap::helper::hash_combine(vertexHash, materialIndex);
	...
	{
		ap::helper::messageBox("OBJ import failed! Check backlog for errors!", "Error!");
	}
}
