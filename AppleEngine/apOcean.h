...
namespace ap
{
	...
	public:
		...
		void UpdateDisplacementMap(const OceanParameters& params, ap::graphics::CommandList cmd) const;
		void Render(const ap::scene::CameraComponent& camera, const OceanParameters& params, ap::graphics::CommandList cmd) const;

		const ap::graphics::Texture* getDisplacementMap() const;
		const ap::graphics::Texture* getGradientMap() const;
	...
	protected:
		ap::graphics::Texture displacementMap;		// (RGBA32F)
		ap::graphics::Texture gradientMap;			// (RGBA16F)
		...
		ap::graphics::GPUBuffer buffer_Float2_H0;

		// Angular frequency
		ap::graphics::GPUBuffer buffer_Float_Omega;

		// Height field H(t), choppy field Dx(t) and Dy(t) in frequency domain, updated each frame.
		ap::graphics::GPUBuffer buffer_Float2_Ht;

		// Height & choppy buffer in the space domain, corresponding to H(t), Dx(t) and Dy(t)
		ap::graphics::GPUBuffer buffer_Float_Dxyz;


		ap::graphics::GPUBuffer immutableCB;
		ap::graphics::GPUBuffer perFrameCB;
	};
}
